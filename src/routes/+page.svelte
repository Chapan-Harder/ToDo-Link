<script lang="ts">
	let ToDoList: any = []; // Array Of ToDos
...
<h1 class="text-center text-teal-50 text-5xl font-sans font-black capitalize mt-10 mb-20">
	ToDo list
</h1>

<p class="text-lg text-slate-400 font-sans font-normal capitalize mb-4">enter your to-do here</p>
<div class="flex justify-center gap-2 mx-2 mb-7">
	<input
...
{#each ToDoList as ToDo, counter}
	<section class="flex items-baseline gap-2 w-auto my-0 mx-auto mb-2">
		{#if ToDo.editing}
			<input
		...
		<div class="flex gap-1">
			{#if ToDo.editing}
				<button
					class="bg-cyan-600 rounded-md text-white font-sans font-normal capitalize h-11 w-auto p-1"
					on:click={() => setEdeting(counter, false)}>save</button
				>
			{:else}
				<button
					class="bg-cyan-600 rounded-md text-white font-sans font-normal capitalize h-11 w-auto p-1"
					on:click={() => setEdeting(counter, true)}>edit</button
				>
			{/if}
			<button
				class="bg-cyan-600 rounded-md text-white font-sans font-normal capitalize h-11 w-auto p-1"
				on:click={() => deleteToDo(counter)}>delete</button
			>
		</div>
		<!-- Buttons End -->
	</section>
